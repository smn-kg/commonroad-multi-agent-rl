...
def main():
    ...
    metric = metrics[2] # Select the metric to plot
    ...
    for run_group_name in run_group_names:
        print(f"Processing run group: {run_group_name}")
        run_group_data = process_run_group(api, entity, project, run_group_name, metric)

        collected_metric_data[run_group_name] = run_group_data
    ...
    for name, data in collected_metric_data.items():
        x = data.global_step.iloc[:, 0] * step_factor  * 400_000 / 409_000
        plt.plot(x, data[metric + "_mean"], label=f"{map_run_group_name_to_string[name]}")
    ...
    plt.ylabel(map_metric_to_string[metric])
    plt.xlim(left=0)
    plt.xlim(right=400_000 * step_factor)
    plt.title(f"{map_metric_to_string[metric]} with bootstrapped 95% confidence interval", pad=15, fontsize=14)
    plt.legend(
        loc="upper center",
        bbox_to_anchor=(0.5, -0.15),  # center below the axis
        ncol=len(collected_metric_data),  # number of columns = number of legend entries
        frameon=False
    )
    # plt.grid(True)
    plt.tight_layout(pad=0.5)
    plt.tick_params(width=0.8, length=4.5)
    plt.savefig(current_dir / "output" / f"bootstrapped_ci_{metric.replace('/', '_')}.svg", bbox_inches="tight", dpi=300)
    plt.show()
